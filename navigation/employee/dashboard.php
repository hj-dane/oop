...
<head>
  ...
  <title>InSync – My Performance</title>
...
<div class="sidebar" id="sidebar">
  ...
  <a href="dashboard.php" class="link-active mb-2 mt-3">
  ...
  <div class="container mt-4">
    ...
    <div class="welcome-banner">
      <div>
        ...
        <p class="mb-1">
          See how you are performing on your assigned projects and tasks.
        </p>
      </div>
      <div class="text-end">
        ...
        <button id="btnPrintReport" class="btn btn-light btn-sm btn-print-report">
          <i class="fa fa-print me-1"></i> Print My Performance
        </button>
      </div>
    </div>
      ...
      <div class="row g-2 align-items-end">
        ...
        <div class="col-sm-4 text-sm-end">
          <small class="text-muted d-block mb-1">
            Your metrics below update based on the selected date.
          </small>
    ...
    <div class="row mb-2">
      <div class="col-md-3">
        <div class="card-kpi">
          <div class="card-title"><i class="fa fa-gauge-high me-1"></i> My Avg. Score</div>
          <div class="card-value" id="kpi-score">0%</div>
          <div class="card-sub">Average performance score on my tasks</div>
        </div>
      </div>
      <div class="col-md-3">
        <div class="card-kpi">
          <div class="card-title"><i class="fa fa-diagram-project me-1"></i> My Active Projects</div>
          <div class="card-value" id="kpi-projects">0</div>
          <div class="card-sub">Projects where I have active work items</div>
        </div>
      </div>
      <div class="col-md-3">
        <div class="card-kpi">
          <div class="card-title"><i class="fa fa-check-circle me-1"></i> Tasks Completed</div>
          <div class="card-value" id="kpi-employees">0</div>
          <div class="card-sub">Tasks I’ve completed in the selected range</div>
        </div>
      </div>
      <div class="col-md-3">
        <div class="card-kpi">
          <div class="card-title"><i class="fa fa-triangle-exclamation me-1"></i> My Items at Risk</div>
          <div class="card-value" id="kpi-risk">0</div>
          <div class="card-sub">My tasks behind schedule or below target</div>
        </div>
      </div>
    </div>
    ...
    <div class="row">
      ...
      <div class="col-lg-6 mb-3">
        <div class="card-kpi">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <div class="card-title m-0">My Recent Activity</div>
            <small class="text-muted">Latest 10 updates on my tasks</small>
          </div>
...
<script>
  ...
  // -------- User performance dashboard logic ----------
  ...
  const elEmployees = document.getElementById('kpi-employees'); // here: "Tasks Completed"
  ...
  function badgeClass(status){
    ...
    if (status === 'on track' || status === 'completed') return 'text-bg-success';
  ...
  async function loadPerformance(){
    ...
    // Expected JSON from backend (for current user only):
    // {
    //   kpi: { score, projects, tasks_completed, risk },
    //   status: { ontrack, atrisk, delayed, completed },
    //   activity: [{ project, task, status, updated_at }, ...]
    // }
    let payload = {
    ...
    try{
      const res = await fetch('ajax/user_performance_metrics.php?date=' + encodeURIComponent(date), {
      ...
      console.error('user_performance_metrics fetch failed:', e);
    }

    const k = payload.kpi || {};
    elScore.textContent     = (k.score ?? 0) + '%';
    elProjects.textContent  = k.projects        ?? 0;
    elEmployees.textContent = k.tasks_completed ?? 0;
    ...
    if(!items.length){
      activityBody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">No activity yet.</td></tr>';
    }else{
  ...
  document.getElementById('btnPrintReport').addEventListener('click', () => {
    ...
    const completed = elEmployees.textContent;
    ...
    try{
      if(window.statusChartInstance && typeof window.statusChartInstance.toBase64Image === 'function'){
        ...
        chartImg = `<img src="${src}" style="max-width:100%;height:auto;border-radius:10px;border:1px solid #e5e7eb;padding:6px;background:#fff;" alt="My Tasks by Status">`;
      }
    }catch(e){}
...
<head>
<meta charset="utf-8">
<title>InSync – My Performance Report</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
...
<body>
  <div class="wrap">
    <div class="header d-flex justify-content-between align-items-center">
      <div>
        <div class="brand">InSync • My Performance Report</div>
        <div class="sub">Printed: ${now}</div>
      </div>
      <div class="text-end">
        <div><span class="badge-soft">Date filter: ${selDate}</span></div>
        <div class="mt-1"><small>Prepared for: <strong>${userName}</strong></small></div>
      </div>
    </div>

    <div class="card mb-3 p-3">
      <h6 class="mb-3">My Key Metrics</h6>
      <div class="row g-3">
        ...
        <div class="col-sm-6 col-lg-3">
          <div class="border rounded-3 p-3 h-100">
            <div class="text-muted small mb-1"><i class="fa fa-check-circle me-1"></i> Tasks Completed</div>
    ...
    <div class="card mb-3 p-3">
      <h6 class="mb-2">My Tasks by Status</h6>
      ${chartImg || '<div class="text-muted">Chart not available.</div>'}
    </div>

    <div class="card mb-3 p-3">
      <h6 class="mb-2">How to use this report</h6>
      <ul class="mb-0">
        <li>Track your average score and make sure it stays within your team’s target.</li>
        <li>Review active projects and confirm that your tasks are moving on schedule.</li>
        <li>Use the list of items at risk to prioritize what you’ll work on next.</li>
        <li>Share this report with your supervisor during check-ins or evaluation discussions.</li>
      </ul>
    </div>

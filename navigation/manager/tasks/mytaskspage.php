...
<body>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <a href="monitortaskspage.php" class="mb-2 mt-1 text-truncate">
      <i class="fas fa-eye me-2"></i> <span>Monitor Tasks</span>
    </a>
    <a href="mytaskspage.php" class="link-active mb-2 mt-1 text-truncate">
    ...
    <div class="container mt-4">
      <div class="welcome-banner">
        <div>
          ...
          <p class="mb-0">View and update tasks assigned to you.</p>
        </div>
        <button id="printListBtn" class="btn btn-light text-primary border">
          <i class="fa fa-print me-1"></i> Print My Task List
        </button>
      </div>
  ...
  <!-- TASK MODAL (user can change status + remarks) -->
          ...
          <form id="taskForm">
            ...
            <div class="row g-3">
              <div class="col-md-4">
                ...
                <select class="form-select" id="tStatus" name="status" required>
              ...
              <div class="col-md-8">
                <label class="form-label">Remarks / Progress Update</label>
                <textarea class="form-control" id="tRemarks" name="remarks" rows="2" placeholder="Optional remarks..."></textarea>
              </div>
            </div>
          </form>
        </div>
  ...
  <script>
    ...
    $(function(){
      tasksDT = $('#tasksTable').DataTable({
        responsive: true,
        ajax: 'fetch_my_tasks.php',
        order: [[2, 'asc']], // by due date
        columns: [
          {
            data: 'title',
          ...
          { data: 'assigned_by_name' },
          {
            ...
            orderable:false,
            render: row => `
              <button class="btn btn-sm btn-info view-btn" data-id="${row.task_id}">
    ...
    // open modal with row data (no extra AJAX needed)
    $(document).on('click', '.view-btn', function () {
      const tr  = $(this).closest('tr');
      const row = tasksDT.row(tr).data();
      if (!row) return;

      currentTaskRow = row;

      $('#task_id').val(row.task_id);
      $('#tTitle').val(row.title || '');
      $('#tProject').val(row.project_name || '');
      $('#tDueDate').val(row.due_date || '');
      $('#tAssignedBy').val(row.assigned_by_name || '');
      $('#tDescription').val(row.description || '');
      $('#tStatus').val(row.status || 'Pending');
      $('#tRemarks').val('');

      $('#taskModalTitle').text('Update My Task');
    ...
    $('#saveTaskBtn').on('click', function(){
      ...
      const remarks  = $('#tRemarks').val();

      if (!taskId) {
        return Swal.fire('Error','No task selected.','error');
      }
      ...
      $.ajax({
        ...
        if (res.success) {
          Swal.fire('Saved', res.message || 'Task updated.', 'success');
          $('#taskModal').modal('hide');
          $('#tasksTable').DataTable().ajax.reload(null,false);
        } else {
          Swal.fire('Error', res.error || 'Failed to update task.', 'error');
        }
      }).fail(function(){
...
<meta charset="utf-8">
<title>InSync – My Task #${t.task_id}</title>
...
<body>
  <div class="wrap">
    <div class="print-header d-flex justify-content-between align-items-center">
      <div>
        <h5 class="m-0">InSync • My Task</h5>
...
<body>
  <div class="wrap">
    <div class="print-header d-flex justify-content-between align-items-center">
      <div>
        <h5 class="m-0">InSync • My Tasks</h5>
        <div class="small opacity-75">Prepared by: <strong>${esc(me)}</strong></div>
      </div>
      <div class="text-end">
        <div class="small">Printed: ${esc(now)}</div>
    ...
    <div class="card shadow-sm">
      <div class="table-responsive">
        <table class="table table-sm table-striped align-middle m-0">
          ...
          <tbody>${tbody || '<tr><td colspan="5" class="text-center text-muted py-3">No tasks</td></tr>'}</tbody>
        </table>
      </div>
    </div>

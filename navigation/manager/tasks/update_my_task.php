<?php
// navigation/manager/tasks/update_my_task.php
session_start();
ob_start();
...
try {
    ...
    // make sure the task is assigned to this user
    $stmt = $conn->prepare("
    ...
    // record task history (even if status same but there are remarks)
    ...
    /* ========= LOG HISTORY HERE =========
       action_type: use the id from `actions` table
       e.g. 3 = 'Update Task Status'
    ==================================== */
    $ACTION_UPDATE_TASK_STATUS = 3; // adjust if your ID is different
    ...
    // created_at will auto-fill with CURRENT_TIMESTAMP()

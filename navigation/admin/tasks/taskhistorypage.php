...
<body>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <a href="taskspage.php" class="mb-2 mt-1 text-truncate">
      <i class="fas fa-tasks me-2"></i> <span>Monitor Tasks</span>
    </a>
    <a href="taskhistorypage.php" class="link-active mb-2 mt-1 text-truncate">
    ...
    <div class="container mt-4">
      <div class="welcome-banner">
        <div>
          ...
          <p class="mb-0">Read-only history of status changes for all tasks.</p>
        </div>
      ...
      historyDT = $('#historyTable').DataTable({
        ...
        ajax:{
          ...
          error:function(xhr,status,err){
            console.error('Admin task history Ajax error:', status, err);
            console.log('Server response:', xhr.responseText);
            Swal.fire('Error','Failed to load task history. Check console for details.','error');
          }
        },
        order:[[0,'desc']],
        columns:[
  ...
  <div class="wrap">
    <div class="print-header d-flex justify-content-between align-items-center">
      <div>
        ...
        <div class="small opacity-75">Prepared by: <strong><?= htmlspecialchars(trim("$firstname $middlename $lastname")) ?></strong></div>
      </div>
      <div class="text-end">
        <div class="small">Printed: ${esc(now)}</div>
        <div class="small">Date range: ${esc(dateRange)}</div>
        <div class="small">Total rows: ${rows.length}</div>
      </div>
    </div>
    ...
    <div class="card shadow-sm">
      <div class="table-responsive">
        <table class="table table-sm table-striped align-middle m-0">
          ...
          <tbody>${tbody || '<tr><td colspan="8" class="text-center text-muted py-3">No history.</td></tr>'}</tbody>
        </table>
      </div>
    </div>

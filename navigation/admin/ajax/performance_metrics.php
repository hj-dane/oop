...
/* ===========================
   DATE FILTER (one source of truth)
   =========================== */

// What JS sent (raw)
$raw = isset($_GET['date']) ? trim($_GET['date']) : '';
$filterDate = '';

// Accept either YYYY-MM-DD (from <input type="date">) or MM/DD/YYYY
if ($raw !== '') {
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $raw)) {
        $filterDate = $raw; // already ISO
    } elseif (preg_match('/^(\d{2})\/(\d{2})\/(\d{4})$/', $raw, $m)) {
        // mm/dd/yyyy -> yyyy-mm-dd
        $filterDate = "{$m[3]}-{$m[1]}-{$m[2]}";
    }
}

// Reference dates for “risk”
$today    = date('Y-m-d');
...
/* ===========================
   2) Active projects
   (selected date between start_date and end_date)
   =========================== */

$paramsProj = [];
$sql = "SELECT COUNT(*) FROM projects p WHERE p.status <> 'Archived'";
if ($filterDate !== '') {
    // use two placeholders to avoid HY093
...
$kpiProjects = (int)$stmt->fetchColumn();
    ...
    /* ===========================
       ...
       (distinct assigned_to where task due_date hits filter)
       =========================== */
    ...
    /* ===========================
       4) Items at risk
       (non-completed; filtered by due_date if selected)
       =========================== */

    try {
        $paramsRisk = [];
        $sql = "SELECT COUNT(*) FROM tasks t WHERE t.status <> 'Completed'";
        if ($filterDate !== '') {
            $sql .= " AND t.due_date = :d";
    ...
    $paramsChart = [];
    $sql = "SELECT t.status, t.due_date FROM tasks t";
    if ($filterDate !== '') {
        $sql .= " WHERE t.due_date = :d";
    ...
    $sql .= " ORDER BY COALESCE(t.updated_at, t.created_at) DESC LIMIT 10";

...
<head>
  ...
  <title>InSync – Log History</title>
...
<body>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <a href="loghistorypage.php" class="link-active mb-2 mt-1 text-truncate">
  ...
  <div class="content" id="content">
    ...
    <div class="container mt-4">
      <div class="welcome-banner">
        <div>
          <h2 class="mb-1">Log History</h2>
          <p class="mb-0">Read-only audit of important actions performed in the system.</p>
        </div>
        <button id="printListBtn" class="btn btn-light text-primary border">
          <i class="fa fa-print me-1"></i> Print Logs
        </button>
      </div>
        ...
        <div class="row g-2 align-items-end">
          ...
          <div class="col-md-3 text-md-end">
            <small class="text-muted d-block mb-1">
              Showing logs within selected dates.
            </small>
  ...
  <script>
      ...
      logsDT = $('#logsTable').DataTable({
        ...
        ajax:{
          ...
          error:function(xhr,status,err){
            console.error('Log history Ajax error:', status, err);
            console.log('Server response:', xhr.responseText);
            Swal.fire('Error','Failed to load log history. Check console for details.','error');
          }
        },
        order:[[0,'desc']],
        columns:[
    ...
    document.getElementById('printListBtn').addEventListener('click', function(){
      if (!logsDT) return;
      const rows = logsDT.rows({search:'applied'}).data().toArray();
...
<body>
  <div class="wrap">
    <div class="print-header d-flex justify-content-between align-items-center">
      <div>
        <h5 class="m-0">InSync • Log History</h5>
      ...
      <div class="text-end">
        ...
        <div class="small">Total logs: ${rows.length}</div>
      </div>
    </div>
    ...
    <div class="card shadow-sm">
      <div class="table-responsive">
        <table class="table table-sm table-striped align-middle m-0">
          ...
          <tbody>${tbody || '<tr><td colspan="4" class="text-center text-muted py-3">No logs.</td></tr>'}</tbody>
        </table>
      </div>
    </div>
